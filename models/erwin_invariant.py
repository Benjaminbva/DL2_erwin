...
class BallPooling(nn.Module):
    ...
    def forward(self, node: Node) -> Node:
        ...
        with torch.no_grad():
            ...
            dist = torch.norm(pos - centers[:, None], dim=-1)
            rel_pos = rearrange(dist, "n s -> n (s 1)")
...
class BallUnpooling(nn.Module):
    ...
    def forward(self, node: Node) -> Node:
        with torch.no_grad():
            child_pos = rearrange(node.children.pos, "(n m) d -> n m d", m=self.stride)
            dist = torch.norm(child_pos - node.pos[:, None], dim=-1)
            rel_dist = rearrange(dist, "n m -> n (m 1)")
...
class BallMSA(nn.Module):
    ...
    def compute_rel_pos(self, pos: torch.Tensor):
        ...
        return torch.norm((pos - pos.mean(dim=1, keepdim=True)).view(-1, dim), dim = 1, keepdim=True)
