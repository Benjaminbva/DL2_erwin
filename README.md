...
- **Weaknesses**
    - By enforcing full E(3) equivariance with the introduction of GATr, GATrErwin has about ten times more parameters when using a similar architecture. It runs roughly four times slower per training epoch compared to an Erwin model with similar number of parameters. Specifically, in cases where inference speed is the priority, the simpler Erwin model more convenient. However, the extra cost can be justified when datasets are small or when robustness to arbitrary orientations is critical although other equivariant models outperform GATrErwin on smaller data sizes.
